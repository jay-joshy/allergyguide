+++
title = "Disclaimer"
description = "Legal, Licensing, & Disclaimers"
path = "disclaimer"
template = "pages.html"
draft = false
in_search_index = false
+++

# 1. Medical Disclaimer

**This website, its content, and its software tools are for educational and informational purposes only.**

**They do not replace professional medical advice.**

The content is not meant to be complete or exhaustive or to be applicable to any specific individual's medical condition. Software tools provided are intended for clinical decision support only and must be verified by a licensed healthcare professional before use in a clinical setting.

Always consult a licensed healthcare provider for concerns about your health - the content of this website is not a substitute for professional medical advice, diagnosis, or treatment. We assume no legal duty to correct or update the website nor to resolve or clarify any inconsistent information that might be a part of the website.

**No Warranties**: The software and tools are provided "as is," without warranty of any kind, express or implied.

# 2. Open Source Licensing

## Software & Source Code (GNU GPLv3)

The underlying software (available on the GitHub repository) are licensed under the [GNU General Public License v3 (GPLv3)](https://github.com/jay-joshy/allergyguide/blob/main/LICENSE). You are free to use, modify, and distribute the software. If you distribute modified versions of this software, they must remain open-source under the same GPLv3 license.

## Website Content (CC BY 4.0)

The educational text, documentation, and design assets on this website are licensed under [Creative Commons](https://creativecommons.org/licenses/by/4.0/). You are free to share and adapt the material for any purpose, even commercially, provided you give appropriate credit to allergyguide.ca.
